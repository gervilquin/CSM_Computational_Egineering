function Pe = ComputeSurfaceForces(X,p_inf,n_u,n_l,AoA)
    
    N_skin_nodes = length(n_u(:,4))+length(n_l(:,4));
    chord = max(X(:,1))-min(X(:,1));
    span = max(X(:,2))-min(X(:,2));

    Pe = zeros(N_skin_nodes*3,3);

    for i = 1:length(n_l(:,1))
        p_node = ComputePressureSkin(AoA,X(n_l(i,4),1),X(n_l(i,4),2),X(n_l(i,4),3),chord,span,p_inf);
        index_Pe = (i-1)*3;
    ...
    for i = 1:length(n_u(:,1))
        p_node = ComputePressureSkin(AoA,X(n_u(i,4),1),X(n_u(i,4),2),X(n_u(i,4),3),chord,span,p_inf);
